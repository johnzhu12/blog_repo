...
[https://developer.mozilla.org/zh-CN/docs/Web/API/SubtleCrypto](https://developer.mozilla.org/zh-CN/docs/Web/API/SubtleCrypto)
...
翻译过来：
* 明文和密文关系模糊，使攻击者很难分析，比如通过替换等方式实现。
...
* 填充标准
 ...
 最简单的方式填充0，但如果明文本身末尾有0，解密出来的就不是原文了
...
不细讲，只提和hash的区别：
消息认证码(mac)是带密钥的，构造方法上通常基于Hash函数，比如HMAC,MDx-MAC。也可以基于分组密码比如CBC类的MAC，还有就是基于泛Hash函数族。
